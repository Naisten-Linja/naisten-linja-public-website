...
if (!APP) {
  ...
  console.error(
    'Define that in config vars or by activating Heroku Labs feature `runtime-dyno-metadata`',
  );
  process.exit(1);
}
...
async function main() {
  ...
  for (const build of previousStillPending) {
    console.log(
      `Deleting build ${build.id} in ${build.app.id} which was created at ${build.created_at} by ${build.user.email}`,
    );
